<?
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$name["cardid"] = $masterName["cardid"];
		$name["userid"] = $masterName["userid"];
		...
		if(isset($_FILES['image'])){
			if($_FILES['image']['error']==0) {
				...
				$card["logo"] = str_replace("../", "", $folderName).$fileNameNew.'.'.$fileExt;
			...
			//echo "logo",$logo;
			//echo "logo",$card["logo"];
			$_SESSION['editedcard'] = $card;
			$_SESSION['editedphone'] = $phone;
			$_SESSION['editedname'] = $name;
			$_SESSION['isedited'] = $isEdited;

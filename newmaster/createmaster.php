		...
		foreach($phonearr as $key => $val){
			if($_POST[$key]){
				...
				if($phoneidarray["id"]){//if phone exist
				                                        /////check if user wants to create new master under this number
					$phoneId = $phoneidarray["id"];
					$phone["phoneid".$val] = $phoneId;
					                                     ////if phone exist send to ?????Master under this number is exist would you like to create new master?
				}
